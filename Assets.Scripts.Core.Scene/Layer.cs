...
namespace Assets.Scripts.Core.Scene
{
	public class Layer : MonoBehaviour
	{
		...
		private void CreateMesh(int width, int height, Vector2 origin, bool ryukishiClamp, int finalXOffset, bool stretchToFit)
		{
			...
			int num2 = num / height;
			int width2 = stretchToFit ? Mathf.RoundToInt(num * GameSystem.Instance.AspectRatio) : Mathf.RoundToInt((float)Mathf.Clamp(width, 1, num2 * width));
		...
		private void CreateMesh(int width, int height, LayerAlignment alignment, bool ryukishiClamp, int finalXOffset, bool stretchToFit)
		{
			...
			int width2 = stretchToFit ? Mathf.RoundToInt(num * GameSystem.Instance.AspectRatio) : Mathf.RoundToInt(Mathf.Clamp((float)width, 1f, num2 * (float)width));
